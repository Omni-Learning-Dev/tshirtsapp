<div data-animation="default" data-collapse="medium" data-duration="400" data-easing="ease" data-easing2="ease" role="banner" class="navbar w-nav">
    ...
    <nav role="navigation" class="nav-menu w-nav-menu">
        <a href="{{ route('cart') }}" class="nav-link-4 cart-nav w-nav-link {{ request()->routeIs('cart') ? 'w--current' : '' }}">Cart<div class="cart-count">{{ $cartCount }}</div></a>
        <a href="{{ route('about') }}" class="nav-link-4 w-nav-link {{ request()->routeIs('about') ? 'w--current' : '' }}">ABOUT</a>
        <a href="{{ route('how-it-works') }}" class="nav-link-4 w-nav-link {{ request()->routeIs('how-it-works') ? 'w--current' : '' }}">How it works</a>
        <a href="#Contact-Us" class="nav-link-4 w-nav-link">Contact</a>
